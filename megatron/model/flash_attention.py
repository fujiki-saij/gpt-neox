...
def _flash_attn_forward_cuda(q, k, v, out, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
...
def _flash_attn_backward_cuda(dout, q, k, v, out, softmax_lse, dq, dk, dv, cu_seqlens_q, cu_seqlens_k,
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, dropout_p,
        ...
        if softmax_scale is None:
            softmax_scale = q.shape[-1] ** (-0.5)
        out, softmax_lse, rng_state, S_dmask = _flash_attn_forward_cuda(
            q, k, v, torch.empty_like(
                q), cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
    ...
    def backward(ctx, dout, *args):
        ...
        dq, dk, dv = torch.empty_like(
            q), torch.empty_like(k), torch.empty_like(v)
        _flash_attn_backward_cuda(
            dout, q, k, v, out, softmax_lse, dq, dk, dv, cu_seqlens_q, cu_seqlens_k,
            ctx.max_seqlen_q, ctx.max_seqlen_k, ctx.dropout_p, ctx.softmax_scale, ctx.causal,
            rng_state=rng_state, num_splits=1 if ctx.deterministic else 0,
...
class FlashAttnQKVPackedSplitFunc(torch.autograd.Function):
    ...
    def forward(ctx, qkv, cu_seqlens, max_seqlen0, max_seqlen1, batch_size0, dropout_p,
        ...
        _, softmax_lse0, S_dmask0 = _flash_attn_forward_cuda(
            qkv[:, 0], qkv[:, 1], qkv[:, 2], out, cu_seqlens[:batch_size0 + 1],
            cu_seqlens[:batch_size0 +
                       1], max_seqlen0, max_seqlen0, dropout_p, softmax_scale,
            causal=causal, return_softmax=return_softmax
        )
        s = torch.cuda.Stream()
        with torch.cuda.stream(s):
            _, softmax_lse1, S_dmask1 = _flash_attn_forward_cuda(
    ...
    def backward(ctx, dout, *args):
        ...
        _flash_attn_backward_cuda(
            ...
            cu_seqlens[:batch_size0 +
                       1], ctx.max_seqlen0, ctx.max_seqlen0, ctx.dropout_p,
...
def flash_attn_unpadded_qkvpacked_func_cuda(qkv, cu_seqlens, max_seqlen, dropout_p, softmax_scale=None,
...
def flash_attn_unpadded_kvpacked_func_cuda(q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
...
def flash_attn_unpadded_func_cuda(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
...
def flash_attn_unpadded_qkvpacked_split_func_cuda(
...
def flash_attn_func(qkv, cu_seqlens, dropout_p, max_s, softmax_scale=None, causal=False,
    ...
    dropout_p should be set to 0.0 during evaluation
    """
    return flash_attn_unpadded_qkvpacked_func_cuda(qkv, cu_seqlens, max_s, dropout_p, softmax_scale,
